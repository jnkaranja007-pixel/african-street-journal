...
function Get-Briefs($country) {
  ...
  for ($turn = 0; $turn -lt 4; $turn++) {
    # max_tokens caps thinking + text TOGETHER. Sonnet 5 runs adaptive thinking by
    # default (Haiku 4.5 does not), so the old 4000 was consumed by reasoning and the
    # model never got to emit the JSON - every country silently returned null. Leave
    # generous room. Thinking stays on: with it disabled Sonnet reaches for the search
    # tool far less, and searching is the entire job here.
    $body = @{
      model = $Model
      max_tokens = 16000
    ...
    # Every bail-out below used to `return $null` in silence, so a country that failed
    # looked identical to one with no news. Say why - a silent null cost a full run.
    if (-not $resp) { Write-Host '    empty response from API' -ForegroundColor Red; return $null }
    ...
    if (-not $text) {
      $kinds = (@($parsed.content) | ForEach-Object { $_.type }) -join ','
      Write-Host "    no text block (stop_reason=$($parsed.stop_reason), blocks=$kinds, out_tokens=$($parsed.usage.output_tokens)) - raise max_tokens?" -ForegroundColor Red
      return $null
    }
    # extract the JSON object
    $s = $text.IndexOf('{'); $e = $text.LastIndexOf('}')
    if ($s -lt 0 -or $e -le $s) {
      Write-Host "    no JSON object in reply (stop_reason=$($parsed.stop_reason), $($text.Length) chars)" -ForegroundColor Red
      return $null
    }
    try { return ($text.Substring($s, $e - $s + 1) | ConvertFrom-Json) }
    catch {
      Write-Host "    JSON parse failed (stop_reason=$($parsed.stop_reason)) - likely truncated by max_tokens" -ForegroundColor Red
      return $null
    }
  }
  Write-Host '    gave up after 4 turns (still paused on server tool loop)' -ForegroundColor Red
  return $null
}
...
# A run that files nothing is a failed run, even though the merged file is still
# structurally valid and the publication gate would happily pass it. Without this,
# a broken desk serves last month's news every night behind a green check and never
# tells anyone. Fail loudly instead; the carried-forward data on disk is untouched.
if ($total -eq 0) {
  Write-Host '[briefs] FAIL: no country produced fresh briefs this run.' -ForegroundColor Red
  Write-Host '[briefs] Every story on the site is carried forward from an earlier run.' -ForegroundColor Red
  Write-Host '[briefs] See the per-country errors above (a truncated reply usually means max_tokens is too low for this model).' -ForegroundColor Red
  exit 1
}
